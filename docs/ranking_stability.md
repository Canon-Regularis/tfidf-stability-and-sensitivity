...
`defined = False`, not `+inf`: `+inf` would silently claim "no constraint" and
...
binary64 and the demonstration has no floating-point content of its own —
scores `0.5` and `0.25`, `m = 0.25`, `ε = 0.125 + 2⁻³⁰`. So `m_k/2` cannot be
improved.
...
`certificate_audit` reports a 2×2 table rather than an accuracy, because accuracy
is the wrong summary — it would reward a certificate that always said "no".
...
One subtlety that decides whether this is a theorem check or a flaky annoyance:
the audit compares the **realised** delta, not the drawn one. `fl(s + d)` rounds,
so actual movement can exceed `|d|` by half an ulp, and the theorem is about the
movement that happened.

## The boundary with A2

Queries with `m_k = 0` are **excluded** from the transition curve and counted
separately. At an exact tie no perturbation is needed to change the outcome —
the result is decided entirely by the tie-break, which is A2's regime. Averaging
