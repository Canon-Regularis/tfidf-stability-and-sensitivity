...
Companion to README §4.1–§4.3. Each bound in §4 is implemented as a *checkable*
object rather than a formula in prose: the code computes both sides and the
property tests search adversarially for a counterexample.
...
adding a document can introduce terms, removing one can retire them. `align_models`
handles this explicitly rather than assuming a shared index space; a naive
implementation that compared vectors positionally would silently compare
different terms.
...
**interaction** term. §4.2 claims the interaction term is a mechanism for
amplification.
...
**query vector itself moves too**. This is easy to miss and would understate the
bound; `run_perturbation` re-embeds the query into each model separately.
...
"not covered by the certificate", never "will change". Measured, ~60% of
uncertified perturbations left the top-k unchanged anyway.

## Property tests as adversarial search

`tests/test_perturbation_bounds.py` uses Hypothesis not to sample typical inputs
but to **search for counterexamples** to §4's inequalities. A bound that holds on
random data and fails on a crafted edge case is not a bound.

Two subtleties that decide whether these tests are theorem checks or flaky
annoyances:

- **Assume on the realised delta, not the drawn one.** The proofs are over the
  reals, but `fl(s + d)` rounds, so realised movement can exceed `|d|` by half an
  ulp. A test that filtered on the drawn `d` would fail intermittently at the
