...
They are deliberately not a restatement of the paper. Where a page overlaps with
a README section it is because the implementation found something the
...
(`struct.pack("<d", x)`), never tolerances. A tolerance would silently permit
exactly the divergence this study is about.
...
The reference-only path is tested in CI, so the claim that it stands alone is
...
**Reduction policy is an explicit parameter, not a hidden default.** `Naive`
(normative, plain left-to-right fold), `Neumaier`, `Pairwise`, `Exact`
(Shewchuk). Making it explicit is what allows the arithmetic noise floor to be
...
- **Numerical error cannot flip a ranking of distinctly-scored documents.** The
  measured noise floor is ~1e−16 and the smallest observed score gap ~7e−10 —
  six orders of magnitude apart. Every ranking instability in the unperturbed
...
benchmark row asserts the second before reporting the first — a speedup on a
wrong answer is worthless. Measured on 2000 documents, |V| = 3428:
...
Fitting has no native counterpart by design: IDF is computed once in exact
decimal arithmetic so the core never evaluates a logarithm ([G13](spec_addenda.md#g13)).
The exact reduction shows no speedup, which is expected — it is dominated by the
same Shewchuk expansion in both languages.
