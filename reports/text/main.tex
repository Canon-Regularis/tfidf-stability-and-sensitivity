...
incidental. First, the numerical noise floor sits roughly six orders of
magnitude below the smallest observed score separation, so floating-point error
...
identify a near-tie rather than construct one. Twenty-three addenda are recorded
...
$\tau$ within it yields bit-identical tie structure. That follows from the
argument above rather than from sampling. Measured: $\eta =
1.665\times10^{-16}$, $\tau_{\text{floor}} = 3.331\times10^{-16}$, $g_{\min} =
6.958\times10^{-10}$, a band of $6.32$ decades containing zero observed gaps.
...
% reports/figures/ is gitignored: figures are regenerated from the recorded
% experiment JSON rather than versioned, so a fresh clone does not have them.
% Without this guard the report fails to build from a clean checkout, which is a
% poor property for the document meant to be the deliverable.
\begin{figure}[ht]\centering
\IfFileExists{../figures/fig_transition.png}{%
  ...
  \texttt{python scripts/run\_stability\_profile.py -o reports/}\\
...
Adjacent gaps are instead either exactly zero (17\% to 18\% of pairs) or above
roughly $10^{-9}$; the interval between is empty. At fine $\tau$ the near-tie
