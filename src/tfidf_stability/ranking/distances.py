...
The paper asks for an ordering distance in two different places and they are not
the same problem:
...
Only one of them is ill-posed, and conflating them produces a quantity that
looks defined but is not.

**Restricted to a tie group, the problem is easy.** A tie group is defined on the
*score* vector, which pi, pi_score and pi_alt all share, so the two orderings
being compared contain exactly the same elements. Plain normalised Kendall tau
applies, and :func:`kendall_tau_distance` computes it by merge-sort inversion
count in ``O(n log n)``.

**Restricted to top-k, it is not.** When ``topk(pi) != topk(pi')`` -- precisely
the case section 7.3 exists to measure -- the two lists rank different sets, and
"the number of discordant pairs" has no meaning. The resolution adopted here is
the **Fagin-Kumar-Sivakumar generalised Kendall distance** (*Comparing Top k
Lists*, SIAM J. Discrete Math. 17(1), 2003), with penalty ``p = 1/2``.

``K^(p)`` is a **near-metric**, not a metric. Measurement against this module's
own implementation shows the triangle inequality failing at every penalty
tested, with the violation growing as ``p`` increases -- so ``p = 1/2`` is
chosen for being the *unbiased* contribution when a list says nothing about a
pair, not for a metric property it does not possess. See
``spec_addenda.md#g2`` for the witness and
...
set -- the set-disagreement indicator section 7.3 explicitly asks for, the
normalised FKS distance, the intersection Kendall with its support size, and
Jaccard -- because each is blind to something the others see. In particular
``K_int`` cannot see membership change at all, which is the effect under study.
"""
...
#:
#: ``p = 1/2`` is the **neutral** choice: knowing nothing about the relative
#: order of two elements absent from a list, they disagree with probability one
#: half, so ``1/2`` is the unbiased estimate of the contribution. ``p = 0``
#: assumes the unseen pair agrees, which biases every measurement *downwards* --
#: the wrong direction for a study whose subject is instability -- and ``p = 1``
#: assumes it always disagrees, biasing upwards.
#:
...
#: and the violation grows with ``p`` (see ``spec_addenda.md#g2``). ``K^(p)`` is
#: a *near*-metric: bounded distortion, not the triangle inequality. That is
#: sufficient for reporting disagreement rates, and it is why this module never
#: clusters on the quantity or treats it as a norm.
...
def inversion_count(sequence: Sequence[int]) -> int:
    ...
    Merge sort, ``O(n log n)``. The naive ``O(n^2)`` enumeration is fine for a
    ...
    -- so the asymptotics are load-bearing here even though they are not in
    :func:`kendall_fks`.
    """
    work = list(sequence)
    buffer = [0] * len(work)
...
def kendall_tau_distance(a: Sequence[int], b: Sequence[int]) -> float:
    ...
    Raises:
        ValueError: If the two orderings do not rank the same set. That is not a
            tolerable input here -- it is the signal that :func:`kendall_fks` is
            the function actually wanted.
    """
...
def fks_max(k: int, penalty: float = FKS_PENALTY) -> float:
    ...
    Note the ``k = 1`` case is *not* degenerate: two disjoint singleton lists
    still contribute one case-3 pair, so the maximum is 1, and the formula gives
    that correctly. Only ``k = 0`` has no pairs at all.
    """
    if k < 1:
        return 0.0
...
def kendall_fks(
    ...
    *,
    ...
    Cases 2 and 3 are where the generalisation lives. They encode the natural
    reading that an element *present* in a list is ranked above one that is
    absent from it: in case 3 the two lists therefore disagree unavoidably, and
    in case 2 they disagree exactly when the list holding both puts the absent
    element first.

    Computed by direct ``O(k^2)`` enumeration over the union. At ``k <= 50``
    that is at most 4950 pairs, and being obviously correct by inspection is
    worth more here than the asymptotics -- the test suite cross-checks it
    against :func:`kendall_tau_distance` on the cases where both apply.
    ...
    def case_two(both: dict[int, int], other: dict[int, int], x: int, y: int) -> float:
        ...
        An element present in a list is taken to be ranked above one absent from
        it, so the two lists disagree exactly when `both` puts the element that
        `other` is missing first.
        """
...
def top_k_disagreement(a: Sequence[int], b: Sequence[int]) -> bool:
    ...
    Deliberately about the set, not the order: section 7.3 measures "the fraction
    of queries for which the top-k set differs", and a pure reordering within an
    unchanged set is a different phenomenon, captured by the FKS distance.
    """
    return set(a) != set(b)
...
class TopKComparison:
    ...
    Reported together because each is blind to something the others see. In
    particular ``kendall_intersection`` cannot detect membership change at all
    -- it restricts to the elements the two lists share -- so quoting it alone
    would understate exactly the effect section 7.3 is measuring.
    """
    ...
    jaccard: float
    #: Number of documents that entered or left the top-k, halved -- i.e. how
    #: many swaps occurred.
    swapped: int
