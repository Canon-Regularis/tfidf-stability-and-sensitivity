...
from tfidf_stability.utils.validation import ChainInflationWarning, TauExceedsScoreRangeWarning
__all__ = ['DEFAULT_RHO_WARN', 'Interval', 'TieGroupIndex', 'chain_inflation_ratio', 'tie_ball_interval', 'tie_chains', 'tie_cliques']
...
class TieGroupIndex:
    ...
    @classmethod
    def build(cls, sorted_scores: Sequence[float], tau: float, *, rho_warn_threshold: float=DEFAULT_RHO_WARN) -> TieGroupIndex:
        """Compute chains and cliques, and emit the two diagnostics."""
        scores = tuple(sorted_scores)
        index = cls(sorted_scores=scores, tau=tau, chains=tie_chains(scores, tau), cliques=tie_cliques(scores, tau))
        if scores:
            span = scores[0] - scores[-1]
            if tau >= span and (not (span == 0.0 and tau == 0.0)):
                ratio = math.inf if span == 0.0 else tau / span
                warnings.warn(f'tau={tau!r} covers the entire score range (span={span!r}, tau/span={ratio!r}): every tie ball is the whole corpus. This is a legitimate point at the top of a sweep, but results here are degenerate and should be marked as such.', TauExceedsScoreRangeWarning, stacklevel=2)
        rho = index.rho
        if math.isfinite(rho) and rho > rho_warn_threshold:
            warnings.warn(f'chain inflation rho={rho:.3g} exceeds {rho_warn_threshold}: the largest tie chain ({index.largest_chain} documents) is far wider than the largest mutually-indistinguishable set ({index.largest_clique}). Tie-group statistics at tau={tau!r} are dominated by transitive chaining.', ChainInflationWarning, stacklevel=2)
        return index
    ...
    def report(self) -> dict[str, object]:
        ...
        """
        return {'tau': self.tau, 'n_documents': len(self.sorted_scores), 'n_chains': self.n_chains, 'largest_chain': self.largest_chain, 'n_cliques': len(self.cliques), 'largest_clique': self.largest_clique, 'rho': self.rho}